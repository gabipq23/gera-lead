...
export const useChatContext = () => {
  ...
  const chatsQuery = useQuery({
    queryKey: ["chats", selectedClient?.id],
    queryFn: async () => {
      ...
      try {
        const response = await messagesService.fetchAll(selectedClient.id);
  ...
  const messagesQuery = useQuery({
    queryKey: ["messages", selectedClient?.id, selectedChatId],
    queryFn: async () => {
      ...
      try {
        const response = await messagesService.fetchAllMessagesFromProspect(selectedChat.id);
  ...
  const sendMessage = useCallback(async (prospectId: string, content: string) => {
  ...
  const isLoadingMessages = selectedChatId ? 
    loading.fetchingMessages[selectedChatId] || messagesQuery.isLoading : false;
  
  const chatError = errors.chatError || (chatsQuery.error as Error)?.message;
  const messageError = selectedChatId ? 
    errors.messageErrors[selectedChatId] || (messagesQuery.error as Error)?.message : null;
