...
export const useMessageSocket = () => {
  const { addMessage, addNewChat, pauseConversation, resumeConversation } =
    useChatContext();
  const { user, selectedClient } = useAuthContext();
  ...
  const playNotificationSound = useCallback(() => {
    ...
    if (localUser?.allowSoundNotification && soundRef.current) {
  ...
  const onMessageUpsert = useCallback(
    (socketMessage: IResponseChatSocket) => {
      try {
        ...
        if (socketMessage.clientId !== selectedClient?.id) return;

        if (localUser?.allowToastNotification) {
          handleNotify(
            `${
              socketMessage.prospect.platformData.name ||
              socketMessage.prospect.externalId
            } te mandou uma nova mensagem`,
            socketMessage.message.data.content,
            socketMessage.prospect.platformData.picture
          );
        }

        if (localUser?.allowSoundNotification) {
    ...
    [addMessage, handleNotify, playNotificationSound, selectedClient]
  );
  ...
  const onNewProspect = useCallback(
    (socketMessage: IResponseChatSocket) => {
      try {
        ...
        // Notificação para novo prospect
        showNotification("Novo prospect adicionado", {
          body: `${
            socketMessage.prospect.platformData?.name || "Nome não disponível"
          } iniciou uma conversa`,
          tag: `new-prospect-${socketMessage.prospect.id}`,
        });
  ...
  const connectSocket = useCallback(() => {
    if (!user?.id) {
      socketLogger.warn("Usuário não autenticado - não conectando socket");
      return;
    }
    ...
    try {
      ...
      socketLogger.log("Tentando conectar socket para usuário:", user.id);
    } catch (error) {
